...
namespace Agm.Controllers
{
    public class TextMessageController : Controller
    {
        ...
        public JsonResult createmsg(int id, string tModel)
        {
            var user = Session["User"] as Users;
            var group = db.Groups.FirstOrDefault(x => x.groupId == id);
        ...
        [HttpGet]
        public void SetSesliYardim()
        {
            Session["SesliYardim"] = true;
        }

        [HttpGet]
        public JsonResult GetSesliYardim()
        { 
            return Json(new { status = Session["SesliYardim"] }, JsonRequestBehavior.AllowGet);
        }
